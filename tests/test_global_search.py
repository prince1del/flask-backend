...
def test_global_search_matches_masters_visit_logs_and_verifications(
    tmp_path: Path,
) -> None:
    ...
    retailer_id = db.add_master_retailer(
        name="Beta Retail", distributor_id=distributor_id, location="Andheri"
    )

    db.add_distributor_visit_log(
        distributor_id, "2026-06-25", "09:00", {"notes": "Alpha discussion"}
    )
    ...
    assert any(
        ...
        for item in results["results"]["verifications"]
    )
    assert any(
        item["category"] == "visit_logs" for item in results["results"]["visit_logs"]
    )
...
def test_global_search_is_case_insensitive(tmp_path: Path) -> None:
    ...
    db.add_master_distributor(
        name="Punjab Traders", gst_no="27BBBB0000A1Z5", zone="North"
    )
