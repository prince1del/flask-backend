...
from app.three_step_verification import _load_product_aliases, _normalize_product_key, compare_step1, compare_step2, compare_step3, run_full_verification
...
def test_run_full_verification_returns_structured_report(tmp_path):
    ...
    so_pdf.write_text("Product: Milk Powder\nQuantity: 12\nRate: 100\nGST: 5\nClient Name: Rahul Kumar Yadav\nInvoice Amount: 1000\nTotal GST: 5", encoding="utf-8")
    invoice_pdf.write_text("Product: Milk Powder\nQuantity: 12\nRate: 100\nGST: 5\nClient Name: Rahul K Yadav\nInvoice Amount: 1000\nTotal GST: 5", encoding="utf-8")
...
def test_compare_step1_matches_rows_by_product(tmp_path):
    ...
    pd.DataFrame([
        ...
        {"product": "Bluemen", "quantity": 24, "rate": 6, "gst": 6, "discount": 250},
    ]).to_excel(order_file, index=False)
...
def test_compare_step1_uses_descriptors_for_same_product_rows(tmp_path):
    ...
    pd.DataFrame([
        {"product": "Bedsheet", "size": "King", "tc": 180, "rate": 2000, "quantity": 100},
        {"product": "Bedsheet", "size": "Queen", "tc": 144, "rate": 1500, "quantity": 80},
    ]).to_excel(order_file, index=False)
...
def test_parse_step1_accepts_headers_on_later_rows(tmp_path):
    ...
    ], dtype=object).to_excel(order_file, header=False, index=False)
...
def test_compare_step3_parses_client_name_without_colon(tmp_path):
    ...
    so_pdf.write_text("Client Name Rahul Kumar Yadav\nInvoice Amount 1000\nTotal GST 5", encoding="utf-8")
    invoice_pdf.write_text("Client Name Rahul Kumar Yadav\nInvoice Amount 1000\nTotal GST 5", encoding="utf-8")
...
def test_compare_step1_returns_error_for_implausible_mapping(tmp_path):
    ...
    pd.DataFrame([
        {"product": "Aster", "quantity": 1728, "rate": 12, "gst": 120, "discount": 1080000},
        {"product": "Bluemen", "quantity": 288, "rate": 6, "gst": 144, "discount": 634176},
    ]).to_excel(filled_file, index=False)

    report = compare_step1(order_file, filled_file)

    assert report["status"] == "error"
    assert any("mapping issue" in message.lower() for message in report.get("errors", []))
...
Item D 63041910 8
    ...
    assert report["status"] == "error"
    assert any("parser issue" in message.lower() for message in report.get("errors", []))
