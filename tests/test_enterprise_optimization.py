...
def test_credit_policy_schema_is_structured_and_bypassable(tmp_path: Path) -> None:
    ...
    policy = db.validate_credit_policy(distributor_id=7, max_credit_limit=50000, credit_days_allowed=30, account_status="ACTIVE")
...
def test_purchase_behavior_logs_aggregate_volume_and_frequency(tmp_path: Path) -> None:
    ...
    tracking_one = db.create_order_lifecycle_tracking(order_ref_no="SO-2001", distributor_id=9, order_received_date="2026-01-01", expected_delivery_date="2026-01-03")
    tracking_two = db.create_order_lifecycle_tracking(order_ref_no="SO-2002", distributor_id=9, order_received_date="2026-01-10", expected_delivery_date="2026-01-12")

    db.record_delivery_receipt(tracking_id=tracking_one, article_id=article_id, invoiced_qty=20, physically_received_qty=20, damaged_qty=0)
    db.record_delivery_receipt(tracking_id=tracking_two, article_id=article_id, invoiced_qty=30, physically_received_qty=30, damaged_qty=0)
