...
pd.DataFrame(
    [{"Product": "ABC", "Quantity": 10, "Rate": 100, "GST": 18, "Discount": 0}]
).to_excel(order_path, index=False)
pd.DataFrame(
    [{"Product": "ABC", "Quantity": 10, "Rate": 100, "GST": 18, "Discount": 0}]
).to_excel(filled_path, index=False)
so_path.write_text("Product: ABC\nQuantity: 10\nRate: 100\nGST: 18", encoding="utf-8")
inv_path.write_text(
    "Client Name: Test Client\nInvoice Amount: 1000\nTotal GST: 180", encoding="utf-8"
)

with open(order_path, "rb") as f1, open(filled_path, "rb") as f2, open(
    so_path, "rb"
) as f3, open(inv_path, "rb") as f4:
    resp = client.post(
        "/",
        data={
