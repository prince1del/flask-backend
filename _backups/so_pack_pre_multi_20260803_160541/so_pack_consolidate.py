...
def analyze_so_pack(file_bytes: bytes, filename: str) -> dict[str, Any]:
    """Parse all SO PDFs in a zip/rar → consolidated / summary / line detail."""
    pdfs = _unpack_archive(file_bytes, filename)
    if not pdfs:
    ...
    with tempfile.TemporaryDirectory(prefix="so_pack_") as tmp:
        tmp_path = Path(tmp)
        for source_name, raw in pdfs:
            pdf_path = tmp_path / source_name.replace("/", "_").replace("\\", "_")
            ...
            # Prefer richer header if later PDF fills gaps
    ...
    # Product consolidation: SO + product_name
