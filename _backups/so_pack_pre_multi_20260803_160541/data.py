...
def so_pack_analyze() -> Response:
    """Unpack ZIP/RAR of SO PDFs → consolidated product qty/amount JSON."""
    ...
    if not (lower.endswith(".zip") or lower.endswith(".rar")):
        return _json_response(
            {"success": False, "error": {"message": "Upload a .zip or .rar of SO PDFs"}},
            400,
        )
    raw = uploaded.read()
    if not raw:
    ...
    try:
        data = analyze_so_pack(raw, fname)
    except ValueError as exc:
