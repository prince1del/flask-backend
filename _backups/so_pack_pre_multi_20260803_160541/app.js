...
const OF_SO_PACK_EXCEL_FORMAT = 'assort-1design-Ncolour-v4';
...
/** White/disabled until Analyze succeeds for the selected pack; then match Analyze (primary) colour. */
function updateSoPackExcelButtonState() {
  ...
  const key = _soPackFileKey(file);
  const ready = Boolean(ofSoPackLastPayload && key && ofSoPackAnalyzedKey === key);
  ...
  } else if (ofSoPackExcelBlob && ofSoPackExcelKey === _soPackExcelCacheKey(key)) {
    btn.title = 'Download Excel (cached — instant)';
  } else {
    btn.title = 'Download Excel for this analyzed pack';
  }
}
...
function _soPackClearPreviewForNewFile() {
  ...
  document.querySelectorAll('[data-of-bottom]').forEach((btn) => {
    const on = btn.getAttribute('data-of-bottom') === 'tracking';
...
function bindSoPackFileInput() {
  ...
  fileInput.addEventListener('change', () => {
    ...
    // New selection must not keep showing a previous pack (e.g. Parnami) on screen.
    if (key !== ofSoPackAnalyzedKey) {
      _soPackClearPreviewForNewFile();
      _soPackShowMessage('New pack selected — click Analyze first.', true);
    }
  });
}
...
function _renderSoPackPreview(data, fileKey) {
...
async function _soPackRunAnalyze(file) {
  const formData = new FormData();
  formData.append('file', file);
  ...
  const payload = data.data || {};
  _renderSoPackPreview(payload, _soPackFileKey(file));
  return payload;
}
...
async function downloadSoPackExcel() {
  ...
  const fileInput = document.getElementById('of-so-pack-file');
  const file = fileInput && fileInput.files && fileInput.files[0];
  const key = file ? _soPackFileKey(file) : ofSoPackAnalyzedKey;
  // Excel must never bypass Analyze — screen and download stay in sync.
  if (!ofSoPackLastPayload || !ofSoPackAnalyzedKey || (key && ofSoPackAnalyzedKey !== key)) {
  ...
  const packKey = ofSoPackAnalyzedKey;
  const excelCacheKey = _soPackExcelCacheKey(packKey);
  // Instant path: workbook already built for this pack + current format — no server round-trip.
  if (ofSoPackExcelBlob && ofSoPackExcelKey === excelCacheKey) {
    _soPackTriggerExcelDownload(ofSoPackExcelBlob, _soPackExcelDownloadName(ofSoPackLastPayload));
    _soPackShowMessage('Excel downloaded (from memory).', true);
    if (typeof nexoraToast === 'function') nexoraToast('Excel downloaded', 'ok');
    return;
  }
  // Drop any stale workbook from an older layout (no totals / freeze).
  _soPackClearExcelCache();
  _soPackShowBusy('excel');
  try {
    let response = null;
    // Prefer analyzed JSON → current build_consolidated_xlsx (totals + freeze). ZIP fallback for older servers.
    if (ofSoPackLastPayload) {
      response = await fetchWithAuth('/api/v1/order-fulfillment/so-pack/excel', {
        ...
        body: JSON.stringify(ofSoPackLastPayload),
      });
    }
    if ((!response || !response.ok) && file) {
      const formData = new FormData();
      formData.append('file', file);
      response = await fetchWithAuth('/api/v1/order-fulfillment/so-pack/excel', {
        method: 'POST',
    ...
    _soPackTriggerExcelDownload(blob, _soPackExcelDownloadName(ofSoPackLastPayload));
